...
func Bannersave(c *fiber.Ctx) error {
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"page":                       client.Page,
			"sdata":                      client.Sdata,
			"moviebanner_id":             client.Moviebanner_id,
...
func Bannerdelete(c *fiber.Ctx) error {
	type payload_delete struct {
		Page          string `json:"page"`
		Sdata         string `json:"sdata" `
		Moviebannerid int    `json:"moviebannerid" `
	}
	hostname := c.Hostname()
	bearToken := c.Get("Authorization")
	token := strings.Split(bearToken, " ")
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"page":            client.Page,
			"sdata":           "New",
			"moviebannerid":   client.Moviebannerid,
		}).
		Post(PATH + "api/moviebannerdelete")
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Response Info:")
